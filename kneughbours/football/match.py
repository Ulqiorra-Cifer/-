...
x_train = np.array([
    ...
    [98,78,13,100000,99,75,14,105000]
], dtype="float32")

y_train=np.array([-1,1,-1,1,1,0,0])

x_train = x_train / np.max(x_train, axis=0)

model = keras.Sequential([
    layers.Dense(16, activation="tanh_shrink", input_shape=(8,)),  # 4 признака
    layers.Dropout(rate=0.3),
    layers.Dense(16, activation="tanh_shrink"),
    layers.Dropout(rate=0.3),
    layers.Dense(1)  # 4 класса погоды
...
model.fit(x_train, y_train, epochs=140, verbose=1)
