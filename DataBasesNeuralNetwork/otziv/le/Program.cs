...
class Program
{
    static void Main()
    {
        ...
        nn.layers.Add(new Layer(new List<decimal>(new decimal[wordIndex.Count]), 6));
        nn.layers.Add(new Layer(6, 1));
...
class neural_network
{
    ...
    private decimal Sigmoid(decimal x)
    {
        double ex = Math.Exp(-(double)x);
        return (decimal)(1 / (1 + ex));
    }
}
